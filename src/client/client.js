...
async function handleStartTaskClick() {
  ...
  if (!prompt && (!files || files.length === 0)) {
    // Require prompt OR files
    addLogMessage(
      "Please enter Your Instructions or provide files.",
      "error",
      true,
    );
    promptInput.focus();
  ...
  if (currentSelectedTaskId === "new") {
    ...
    const existingTask = tasks.find(
      (t) => String(t.id) === String(currentSelectedTaskId),
    );
    ...
    if (existingTask) {
      ...
      if (needsUpdate) {
        ...
        taskToRun = {
          ...
          temperature,
          title: taskTitle,
        }; // Reflect update locally for this run
      } else {
      ...
      addLogMessage(
        `Error: Could not find selected task ${currentSelectedTaskId}. Please select "New Task" or an existing task.`,
        "error",
        true,
      );
  ...
  if (!continueContext) {
    ...
    addLogMessage(
      "ℹ️ Continue Context is enabled. Previous context (if any) should be visible.",
      "info",
      true,
    );
  }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (uploadedFileNames.length > 0) {
          addLogMessage(
            `Uploaded file references: ${uploadedFileNames.join(", ")}`,
            "debug",
          );
        }
      } else {
        addLogMessage(
          `❌ Upload failed: ${result.message || response.statusText}`,
          "error",
          true,
        );
      ...
      addLogMessage(
        `❌ Network error during upload: ${error.message}. Check server connection.`,
        "error",
        true,
      );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!baseDirInput || !promptInput || !startButton || !logOutput) {
    console.error(
      "FATAL: Core UI elements (baseDir, prompt, startButton, logOutput) not found on page load. Application might not function.",
    );
