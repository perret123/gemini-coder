...
async function sendMessageWithRetry(chatSession, message, socket, generationConfig, toolConfig, maxRetries = 3, initialDelay = 120000) {
    let lastError = null;
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        ...
        try {
            ...
            emitLog(socket, ` Gemini API Call (Attempt ${attempt + 1}/${maxRetries})`, "debug");
            const result = await chatSession.sendMessage(messageToSend /*, {generationConfig, toolConfig} - Config passed during startChat */); // Pass message array
            ...
            if (isBadRequestJsonError) {
                ...
                emitContextLogEntry(socket, 'error', `API Bad Request: ${errorMessage}`); // Log to client context
            ...
            if (shouldRetry && attempt < maxRetries - 1) {
                ...
                emitContextLogEntry(socket, 'api_retry', `API Error (${errorType}). Retrying (${attempt + 1}).`); // Update context
                ...
                continue; // Go to next iteration
                 ...
                 if (isInternalOrUnavailable && statusCode === 500) {
                      emitLog(socket, `🧱 Persistent API Error (Status: 500) after ${maxRetries} attempts. Signaling for potential auto-continuation.`, "warn", true);
                     throw new PersistentInternalServerError(`API Error: Persistent 500 after ${maxRetries} attempts. (Details: ${errorMessage})`);
                 }
...
export async function runGeminiTask(context, state) {
    const {
        ...
        chatSession,
        functionHandlers,
        confirmAllRef, // These are refs (objects with .value)
        feedbackResolverRef,
        questionResolverRef,
        temperature,
    ...
    emitLog(socket, `Generation Config: ${JSON.stringify(generationConfig)}`, "debug"); // Log the config used
    ...
    try {
        ...
        while (true) {
            ...
            if (socket.disconnected) {
                ...
                emitContextLogEntry(socket, 'disconnect', 'Task aborted due to disconnect.');
                break;
            }
            ...
            try {
                ...
                if (apiError instanceof PersistentInternalServerError && !isAutoContinuing) {
                    ...
                    emitContextLogEntry(socket, 'api_retry', 'Persistent 500. Attempting auto-resume.');
                    ...
                    if (!originalPromptForState) {
                         ...
                         socket.emit("task-error", { message: `Failed to auto-resume: Internal state error (missing original prompt). Details: ${apiError.message}`, originalPromptForState: originalPromptForState });
                         break;
                    }
                    ...
                    const savedState = taskStates.get(BASE_DIR);
                     if (!savedState || !savedState.originalPrompt) {
                         ...
                         socket.emit("task-error", { message: `Failed to auto-resume: Internal state error (cannot retrieve valid saved state). Details: ${apiError.message}`, originalPromptForState: originalPromptForState });
                         break;
                     }
                    ...
                    socket.emit("task-error", { message: `${failureReason}: ${apiError.message}`, originalPromptForState: originalPromptForState });
                    break; // Stop the task
                }
            }
            ...
            try {
                ...
                try {
                    responseText = response?.candidates?.[0]?.content?.parts?.[0]?.text ?? ".(Could not extract response text).";
                     ...
                     responseText = ".(Response parsing failed completely).";
                 }
                emitLog(socket, `💬 Gemini Raw Response Text (on error): ${responseText}`, "gemini-resp");
                emitContextLogEntry(socket, 'error', `Response Processing Error: ${processingError.message}`);
            ...
            if (functionCalls && functionCalls.length > 0) {
                ...
                for (const call of functionCalls) {
                    // Check for pending user interactions *before* executing the next function
                     if (feedbackResolverRef?.value || questionResolverRef?.value) {
                         ...
                         emitContextLogEntry(socket, 'user_wait', `Waiting for user ${pendingType}`);
                    ...
                    if (socket.disconnected) { // Check connection again inside loop
                    ...
                    if (!handler) {
                        emitLog(socket, ` ⚠️ Function handler not found: ${functionName}. Skipping.`, "warn", true);
                        emitContextLogEntry(socket, 'error', `Function handler not found: ${functionName}`);
                         // Provide an error response back to Gemini
                        functionResponses.push({
                            functionName: functionName, // Echo the name
                            response: { error: `Function handler "${functionName}" not found on server.` }
                        });
                    ...
                    try {
                        ...
                        emitLog(socket, ` ✔️ Result [${functionName}]: ${JSON.stringify(functionResult)}`, "func-result");
                         ...
                         if (functionName === "task_finished" && functionResult?.finished) {
                             ...
                             emitContextLogEntry(socket, 'task_finished', `Finished: ${functionResult.message}`);
                        ...
                        emitContextLogEntry(socket, 'error', `Exec Error (${functionName}): ${execError.message}`);
                ...
                if (functionResponses.length > 0) {
                    ...
                    continue; // Continue the while loop to send results back
                     ...
                     emitContextLogEntry(socket, 'error', 'Function call sequence failed or halted.');
            ...
            // --- Handle Text Response ---
                 ...
                 emitContextLogEntry(socket, 'warning', `Unexpected text response: ${responseText.substring(0, 50)}...`);
            ...
            // --- Handle Empty Response ---
                ...
                // No function calls and no text - model finished its turn without action.
                 emitLog(socket, "🤔 Gemini finished turn without providing text or function calls.", "warn", true);
                 emitContextLogEntry(socket, 'task_finished', 'Finished (No further action)');
