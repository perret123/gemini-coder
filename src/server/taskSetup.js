...
export async function setupAndStartTask(socket, data, state) {
  ...
  if (!userPrompt || !relativeBaseDir) {
    ...
    emitContextLogEntry(
      socket,
      "error",
      `Task Start Failed: ${missing} required.`,
    );
  ...
  try {
    ...
    emitContextLogEntry(
      socket,
      "initial_state",
      `Task Starting. Base Dir: ${BASE_DIR}`,
    );
    emitContextLogEntry(
      socket,
      "initial_prompt",
      `Prompt: ${userPrompt.substring(0, 100)}${userPrompt.length > 100 ? "..." : ""}`,
    );
    ...
    emitLog(
      socket,
      `✅ Base directory confirmed: ${BASE_DIR}`,
      "success",
      true,
    );
    ...
    emitLog(
      socket,
      `Scanning directory structure (max depth 2)...`,
      "info",
      true,
    );
    const structureLines = await getDirectoryStructure(
      BASE_DIR,
      BASE_DIR,
      ig,
      2,
    ...
    emitLog(
      socket,
      `Initial Directory Structure (filtered):\n${structureString || "(empty or all ignored)"}`,
      "info",
      true,
    );
    ...
    if (Array.isArray(uploadedFiles) && uploadedFiles.length > 0) {
      emitLog(
        socket,
        `Processing ${uploadedFiles.length} uploaded image reference(s)...`,
        "info",
        true,
      );
      emitContextLogEntry(
      ...
      for (const filename of uploadedFiles) {
        ...
        if (
          ...
          emitLog(
            socket,
            ` - ⚠️ Skipping potentially unsafe filename "${filename}".`,
            "warn",
            true,
          );
          continue;
        ...
        try {
          ...
          if (mimeType.startsWith("image/")) {
            ...
            emitLog(
              socket,
              ` - Added image "${filename}" (${mimeType}, ${Math.round(fileBuffer.length / 1024)} KB).`,
              "info",
              true,
            );
          } else {
            emitLog(
              socket,
              ` - ⚠️ Skipping non-image file "${filename}" (type: ${mimeType}).`,
              "warn",
              true,
            );
          }
          ...
          if (fileError.code === "ENOENT") {
            emitLog(
              socket,
              ` - ⚠️ Error: Uploaded file reference not found on server: "${filename}". Skipping.`,
              "error",
              true,
            );
          } else {
            emitLog(
              socket,
              ` - ⚠️ Error processing uploaded file "${filename}": ${fileError.message}`,
              "error",
              true,
            );
          }
          emitContextLogEntry(
            socket,
            "warning",
            `Image Processing Error: ${filename} - ${fileError.message}`,
          );
        }
      }
      emitLog(
        socket,
        `Processed ${imageParts.length} valid image(s) for Gemini input.`,
        "info",
        true,
      );
    }
    ...
    if (continueContext && activeChatSessions.has(socket.id)) {
      ...
      emitLog(
        socket,
        "🔄 Continuing previous active chat session (in-memory).",
        "info",
        true,
      );
      // Construct message for continuing task
      initialMessageParts = [
        { text: `User Request (Continue Task): "${userPrompt}"` },
        ...imageParts,
      ];
      ...
      emitLog(
        socket,
        `🔄 Resuming task from saved state for ${BASE_DIR}. Original Goal: "${savedState.originalPrompt}"`,
        "info",
        true,
      );
      ...
      emitContextLogEntry(
        socket,
        "resume_prompt",
        `Resuming Task (Original: ${savedState.originalPrompt.substring(0, 50)}...)`,
      );
      ...
      if (activeChatSessions.has(socket.id)) {
        emitLog(
          socket,
          "🧹 Clearing previous in-memory session before resuming from saved state.",
          "debug",
        );
    ...
    if (contextChangesToSend.length > 0) {
      emitLog(
        socket,
        `📊 Sending initial context state to client (changes: ${contextChangesToSend.length})`,
        "info",
        true,
      );
    ...
    if (
      error.code === "ENOENT" &&
      (error.path === BASE_DIR || error.syscall === "stat")
    ) {
    ...
    emitLog(
      socket,
      `❌ Error during task setup for ${targetDir}: ${error.message}`,
      "error",
      true,
    );
    ...
    if (activeChatSessions.has(socket.id)) {
      activeChatSessions.delete(socket.id);
      console.log(
        `🧹 Cleared active chat session for ${socket.id} due to setup error.`,
      );
    }
  }
}
