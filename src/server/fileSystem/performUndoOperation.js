...
export async function performUndoOperation(operation, context) {
    ...
    // Log the attempt
    const operationTarget = operation.filePath || operation.directoryPath || `${operation.sourcePath} -> ${operation.destinationPath}`;
    emitLog(socket, `⏪ Attempting to undo operation: ${operation.type} (${operationTarget})`, "info");
    ...
    try {
        ...
        switch (operation.type) {
            ...
            case "deleteDirectory":
                 // As noted before, reliable undo is very hard.
        ...
        emitLog(socket, ` ❌ CRITICAL ERROR during undo operation ${operation.type} for ${operationTarget}: ${undoError.message}`, "error");
