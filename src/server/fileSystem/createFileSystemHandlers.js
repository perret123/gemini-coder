...
export function createFileSystemHandlers(context, changesLog) {
    ...
    async function readFileContent(args) {
        ...
        try {
            ...
            if (error.code === "ENOENT") {
                ...
                emitContextLogEntry(socket, "error", `Read Failed (Not Found): ${filePath}`);
            ...
            emitContextLogEntry(socket, "error", `Read Failed: ${filePath} - ${error.message}`);
    ...
    async function listFiles(args) {
        ...
        const logPath = relativeDirectoryPath === "." ? path.basename(BASE_DIR) : relativeDirectoryPath;
        emitContextLogEntry(socket, "listFiles", `List: ${logPath}`);

        try {
            emitLog(socket, ` Ls Listing files in: ${logPath} (respecting .gitignore)`, "info");
            ...
            emitLog(socket, ` ✅ Found ${files.length} entries (after filtering) in ${logPath}`, "info");
            return { success: true, files };
        } catch (error) {
            if (error.code === "ENOENT") {
                emitLog(socket, ` ⚠️ Directory not found: ${logPath}`, "warn");
                emitContextLogEntry(socket, "error", `List Failed (Not Found): ${logPath}`);
                return { error: `Directory not found: '${relativeDirectoryPath}'` };
            }
            emitLog(socket, `❌ Error listing files in ${logPath}: ${error.message}`, "error");
            emitContextLogEntry(socket, "error", `List Failed: ${logPath} - ${error.message}`);
     ...
     async function searchFiles(args) {
        ...
        if (pattern.includes("..")) {
            ...
            emitContextLogEntry(socket, "error", `Search Error: Invalid pattern ${pattern}`);
            return { error: msg };
        }
        if (path.isAbsolute(pattern)) {
             const msg = `Access denied: Search pattern must be relative to the base directory. Pattern: ${pattern}`;
            emitLog(socket, `🔒 SECURITY WARNING: ${msg}`, "warn");
            emitContextLogEntry(socket, "error", `Search Error: Absolute pattern ${pattern}`);
             return { error: msg };
        }

        emitContextLogEntry(socket, "searchFiles", `Search Files (Glob): ${pattern}`);
        ...
        try {
            ...
            emitLog(socket, `❌ Error searching files with pattern '${pattern}': ${error.message}`, "error");
             emitContextLogEntry(socket, "error", `Search Files Failed: ${pattern} - ${error.message}`);
    ...
    async function searchFilesByRegex(args) {
        ...
        const logPath = relativeSearchDir === "." ? path.basename(BASE_DIR) : relativeSearchDir;
        emitContextLogEntry(socket, "searchFilesByRegex", `Search Content (Regex): ${regexString} in ${logPath}`);


        try {
            emitLog(socket, `🔎 Searching file content with regex: ${regex} in ${logPath} (respecting .gitignore)`, "info");
            const ig = await loadGitignore(BASE_DIR, socket);
            const matchingFiles = [];
            let filesScanned = 0;
            let filesIgnored = 0;
                    ...
                    for (const entry of entries) {
                        ...
                        if (entry.isFile()) {
                            ...
                            try {
                                ...
                                if (readError.code !== "ENOENT") { // Ignore if file vanished mid-scan
            ...
            emitLog(socket, ` ✅ Regex search complete. Found ${matchingFiles.length} file(s) with matches. Scanned: ${filesScanned}, Ignored: ${filesIgnored}, Errored: ${filesErrored}.`, "info");
            ...
            emitContextLogEntry(socket, "error", `Search Content Failed: ${regexString} - ${error.message}`);
    ...
    async function writeFileContent(args) {
        ...
        if (!confirmAllRef.value) {
            ...
            emitContextLogEntry(socket, "confirmation_request", `Confirm ${action}: ${filePath}`);
        ...
        const writeResult = await _writeFile(fullPath, contentToWrite, { ...handlerContext, oldContent }, filePath, fileExisted);
    ...
    async function deleteFile(args) {
        ...
        try {
            ...
            if (!stats.isFile()) {
                emitLog(socket, ` ⚠️ Path is not a file, cannot delete: '${filePath}'.`, "warn");
                 emitContextLogEntry(socket, "error", `Delete Failed (Not a File): ${filePath}`);
            ...
            if (readError.code === "ENOENT") {
                ...
                emitLog(socket, ` ⚠️ Error reading file content before deletion (${filePath}): ${readError.message}. Cannot guarantee undo.`, "warn");
                 emitContextLogEntry(socket, "error", `Delete Error (Read Failed): ${filePath} - ${readError.message}`);
        ...
        // --- Confirmation Step ---
        if (!confirmAllRef.value && fileExists) { // Only ask if file exists
            emitContextLogEntry(socket, "confirmation_request", `Confirm Delete: ${filePath}`);
            const userDecision = await requestUserConfirmation(
                socket,
        ...
        } else if (fileExists) { // Skip confirmation log if file didn't exist
     ...
     async function moveItem(args) {
        ...
        if (normSource === normDest) {
            emitLog(socket, ` ⚠️ Source and destination paths are identical: '${normSource}'. No move needed.`, "warn");
            emitContextLogEntry(socket, "moveItem", `Move Skipped (Same Path): ${normSource}`);
        ...
        try {
            ...
            if (accessError.code === 'ENOENT') {
                emitLog(socket, ` ⚠️ Source path not found for move: '${normSource}'. Cannot proceed.`, "warn");
                emitContextLogEntry(socket, "error", `Move Failed (Source Not Found): ${normSource}`);
                return { error: `Source path not found: '${normSource}'` };
            } else {
                // Other access error (permissions?)
                 emitLog(socket, ` ⚠️ Error accessing source path '${normSource}': ${accessError.message}`, "error");
                 emitContextLogEntry(socket, "error", `Move Failed (Source Access Error): ${normSource} - ${accessError.message}`);
                return { error: `Error accessing source path '${normSource}': ${accessError.message}` };
            }
        }
        ...
        try {
            ...
            emitContextLogEntry(socket, "error", `Move Failed (Destination Exists): ${normDest}`);
            return { error: msg };
            ...
            if (destAccessError.code !== "ENOENT") {
                ...
                emitLog(socket, ` ⚠️ Error checking destination path '${normDest}': ${destAccessError.message}`, "warn");
                 emitContextLogEntry(socket, "error", `Move Failed (Dest Access Error): ${normDest} - ${destAccessError.message}`);
                return { error: `Failed to check destination path '${normDest}': ${destAccessError.message}` };
            }
        ...
        // --- Confirmation Step ---
        if (!confirmAllRef.value) {
             emitContextLogEntry(socket, "confirmation_request", `Confirm Move: ${normSource} -> ${normDest}`);
            const userDecision = await requestUserConfirmation(
                socket,
            ...
            if (userDecision === "no" || userDecision === "disconnect" || userDecision === "error" || userDecision === "task-end") {
                const reason = userDecision === "no" ? "rejected" : `cancelled (${userDecision})`;
                emitLog(socket, ` 🚫 Operation cancelled by user/system: moveItem(${normSource}, ${normDest}) - Reason: ${reason}`, "warn");
                emitContextLogEntry(socket, "confirmation_response", `Move ${normSource} -> ${normDest} - ${reason}`);
                return { error: `User or system ${reason} moving item '${normSource}'.` };
            } else if (userDecision === "yes/all") {
                if (confirmAllRef) confirmAllRef.value = true;
                emitLog(socket, ` 👍 Confirmation set to 'Yes to All' for this task.`, "info");
                emitContextLogEntry(socket, "confirmation_response", `Move ${normSource} -> ${normDest} - Confirmed (Yes to All)`);
            } else { // Assume 'yes'
                emitContextLogEntry(socket, "confirmation_response", `Move ${normSource} -> ${normDest} - Confirmed (Yes)`);
            }
        } else {
            emitLog(socket, ` 👍 Skipping confirmation for moving '${normSource}' due to 'Yes to All'.`, "info");
            emitContextLogEntry(socket, "moveItem", `Move: ${normSource} -> ${normDest} (Auto-confirmed)`);
        }

        // --- Execution Step ---
        emitLog(socket, `🚚 Executing move from: ${normSource} To: ${normDest}`, "info");
        // Pass normalized paths to internal function for logging consistency
        const moveResult = await _moveItem(fullSourcePath, fullDestPath, handlerContext, normSource, normDest);
        ...
        if (moveResult.success) {
            ...
            emitLog(socket, ` ❌ Failed move for ${normSource} -> ${normDest}. Error: ${moveResult.error}`, "error");
            // Context log already emitted by confirmation or skip logic, error logged here
            emitContextLogEntry(socket, "error", `Move Failed: ${normSource} -> ${normDest} - ${moveResult.error}`);
            return moveResult;
        }
    }
     ...
     async function createDirectory(args) {
        ...
        emitContextLogEntry(socket, "createDirectory", `Create Folder: ${normDirPath}`);

         // No confirmation needed for createDirectory usually, as it's non-destructive if exists
         // Use the internal function directly
        const createResult = await _createDir(fullPath, handlerContext, normDirPath);

        if (createResult.success) {
            const successMsg = createResult.message || `Directory created successfully at '${normDirPath}'`;
            emitLog(socket, ` ✅ ${successMsg}`, "success");
             // Add context log if it already existed vs created?
            if (createResult.message?.includes("already exists")) {
                emitContextLogEntry(socket, "info", `Folder already exists: ${normDirPath}`);
            } else {
                 // emitContextLogEntry(socket, "createDirectory", `Folder Created: ${normDirPath}`); // Redundant?
            }
            return { success: true, message: successMsg };
        } else {
            emitLog(socket, ` ❌ Failed create directory for ${normDirPath}. Error: ${createResult.error}`, "error");
            emitContextLogEntry(socket, "error", `Create Folder Failed: ${normDirPath} - ${createResult.error}`);
            return createResult;
        }
    }
    ...
    async function deleteDirectory(args) {
        ...
        const normDirPath = path.normalize(directoryPath);
        if (normDirPath === "." || normDirPath === "/" || normDirPath === "" || path.resolve(BASE_DIR, normDirPath) === path.resolve(BASE_DIR)) {
             emitLog(socket, ` ⚠️ Attempted to delete base directory or invalid path: '${normDirPath}'. Denied.`, "error");
             emitContextLogEntry(socket, "error", `Delete Folder Denied (Invalid Path): ${normDirPath}`);
             return { error: `Deleting the base directory or invalid path ('${normDirPath}') is not allowed.` };
        }
        ...
        try {
            ...
            if (!stats.isDirectory()) {
                ...
                emitLog(socket, ` ⚠️ ${msg}`, "warn");
                 emitContextLogEntry(socket, "error", `Delete Folder Failed (Not a Dir): ${normDirPath}`);
                return { error: msg };
            }
            dirExists = true;
        } catch (statError) {
            if (statError.code === "ENOENT") {
                emitLog(socket, ` ⚠️ Directory not found for deletion: '${normDirPath}'. Assuming already deleted.`, "warn");
                emitContextLogEntry(socket, "deleteDirectory", `Delete Folder Skipped (Not Found): ${normDirPath}`);
                return { success: true, message: `Directory '${normDirPath}' not found (already deleted?).` };
            }
            // Other stat errors (permissions?)
            emitLog(socket, `❌ Error accessing directory ${normDirPath}: ${statError.message}`, "error");
             emitContextLogEntry(socket, "error", `Delete Folder Failed (Access Error): ${normDirPath} - ${statError.message}`);
            return { error: `Failed to access directory '${normDirPath}': ${statError.message}` };
        }

        // --- Confirmation Step ---
        if (!confirmAllRef.value && dirExists) { // Only confirm if it exists
            emitContextLogEntry(socket, "confirmation_request", `Confirm Delete Folder (Recursive): ${normDirPath}`);
            const userDecision = await requestUserConfirmation(
                socket,
            ...
            if (userDecision === "no" || userDecision === "disconnect" || userDecision === "error" || userDecision === "task-end") {
                 const reason = userDecision === "no" ? "rejected" : `cancelled (${userDecision})`;
                emitLog(socket, ` 🚫 Operation cancelled by user/system: deleteDirectory(${normDirPath}) - Reason: ${reason}`, "warn");
                emitContextLogEntry(socket, "confirmation_response", `Delete Folder ${normDirPath} - ${reason}`);
                return { error: `User or system ${reason} deleting directory '${normDirPath}'.` };
            } else if (userDecision === "yes/all") {
                if (confirmAllRef) confirmAllRef.value = true;
                emitLog(socket, ` 👍 Confirmation set to 'Yes to All' for this task (including recursive delete).`, "info");
                emitContextLogEntry(socket, "confirmation_response", `Delete Folder ${normDirPath} - Confirmed (Yes to All)`);
            } else { // Assume 'yes'
                 emitContextLogEntry(socket, "confirmation_response", `Delete Folder ${normDirPath} - Confirmed (Yes)`);
            }
        } else if (dirExists) { // Skip confirmation log if dir didn't exist
            emitLog(socket, ` 👍 Skipping confirmation for deleting directory '${normDirPath}' due to 'Yes to All'.`, "info");
            emitContextLogEntry(socket, "deleteDirectory", `Delete Folder: ${normDirPath} (Auto-confirmed)`);
        }
        ...
        emitLog(socket, `🗑️🔥 Executing delete directory (recursive): ${normDirPath}`, "info");
        const deleteResult = await _deleteDirRecursive(fullPath, handlerContext, normDirPath);

        if (deleteResult.success) {
            const successMsg = deleteResult.message || `Directory deleted successfully: '${normDirPath}'`;
            emitLog(socket, ` ✅ ${successMsg}`, "success");
            // Context log handled by confirmation/skip
            return { success: true, message: successMsg };
        } else {
            emitLog(socket, ` ❌ Failed delete directory for ${normDirPath}. Error: ${deleteResult.error}`, "error");
            emitContextLogEntry(socket, "error", `Delete Folder Failed: ${normDirPath} - ${deleteResult.error}`);
            return deleteResult;
        }
    }
    ...
    async function askUserQuestion(args) {
        ...
        return new Promise((resolve) => {
            ...
            socket.emit("ask-question-request", { question });
             ...
             emitContextLogEntry(socket, "answer", `Answer: ${JSON.stringify(answer)}`); // Log answer to context
