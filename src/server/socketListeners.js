...
export function saveCurrentTaskState(socket, state) {
  ...
  const { currentChangesLogRef, currentBaseDirRef, currentOriginalPromptRef } =
    connectionState;
  ...
  if (!baseDir) {
    emitLog(
      socket,
      `ℹ️ Cannot save task state: Base directory not set for this task run.`,
      "info",
    );
  ...
  if (!originalPrompt) {
    ...
    emitLog(
      socket,
      `⚠️ Cannot save task state for ${baseDir}: Original prompt is missing. State NOT saved.`,
      "warn",
      true,
    );
  ...
  if (changes.length > 0 || !existingState || !promptMatches) {
    ...
    if (existingState && promptMatches) {
      ...
      emitLog(
        socket,
        `💾 Merging ${changes.length} new changes with existing state for ${baseDir}. Total changes: ${mergedChanges.length}.`,
        "info",
        true,
      );
      if (changes.length > 0)
        emitLog(
          socket,
          ` (Note: Merged change list may contain redundant operations if task was complex)`,
          "debug",
        );
      stateToSave = {
      ...
      if (existingState && !promptMatches) {
        emitLog(
          socket,
          `💾 Replacing previous state for ${baseDir} due to different original prompt.`,
          "info",
          true,
        );
      } else {
        ...
        emitLog(
          socket,
          `💾 Saving initial task state for ${baseDir} with ${changes.length} changes.`,
          "info",
          true,
        );
      }
    ...
    emitLog(
      socket,
      ` ✅ State saved for base directory: ${baseDir}.`,
      "info",
      true,
    );
    ...
    emitLog(
      socket,
      `ℹ️ No file changes detected in this run segment for ${baseDir}. State not updated.`,
      "info",
    );
  }
  ...
  emitLog(
    socket,
    `🧹 Cleared current change log for connection ${socket.id}.`,
    "debug",
  );
}
...
export function setupSocketListeners(socket, state) {
  const { activeChatSessions, connectionState } = state;
  const {
    feedbackResolverRef,
    questionResolverRef,
  ...
  socket.on("confirmation-response", (data) => {
    ...
    emitLog(
      socket,
      `Received confirmation response: Decision="${decision}", Confirmed=${confirmed}`,
      "info",
    );

    // Check if a resolver function exists in the ref
    if (
      feedbackResolverRef.value &&
      typeof feedbackResolverRef.value === "function"
    ) {
      ...
      emitLog(
        socket,
        `⚠️ Warning: Received confirmation response "${decision}" but no confirmation was actively pending for ${socket.id}. Ignoring.`,
        "warn",
      );
    }
  });
  ...
  socket.on("question-response", (data) => {
    ...
    emitLog(
      socket,
      `Received user question response: ${JSON.stringify(answer)}`,
      "info",
    );

    if (
      questionResolverRef.value &&
      typeof questionResolverRef.value === "function"
    ) {
      ...
      emitLog(
        socket,
        `⚠️ Warning: Received user question response "${JSON.stringify(answer)}" but no question was actively pending for ${socket.id}. Ignoring.`,
        "warn",
      );
    }
  });
  ...
  socket.on("task-complete", (data) => {
    ...
    const promptForState =
      data?.originalPromptForState || currentOriginalPromptRef.value;

    emitLog(
      socket,
      `Internal: task-complete signal received for ${socket.id}. Saving state. Message: ${message}`,
      "debug",
    );
    ...
    if (promptForState && promptForState !== currentOriginalPromptRef.value) {
      emitLog(
        socket,
        `Using original prompt from event data ("${promptForState.substring(0, 30)}..."). instead of ref ("${currentOriginalPromptRef.value?.substring(0, 30)}..."). for state saving.`,
        "debug",
      );
  ...
  socket.on("task-error", (data) => {
    ...
    emitLog(
      socket,
      `Internal: task-error signal received for ${socket.id}. Discarding current changes. Message: ${message}`,
      "debug",
    );
    ...
    emitLog(
      socket,
      `🧹 Discarded unsaved changes for connection ${socket.id} due to error.`,
      "warn",
      true,
    );
    confirmAllRef.value = false; // Reset confirm all flag
  ...
  socket.on("disconnect", (reason) => {
    ...
    if (activeChatSessions.has(socket.id)) {
      ...
      console.log(
        `🧹 Cleared active chat session for disconnected user: ${socket.id}`,
      );
    }
    ...
    if (feedbackResolverRef.value) {
      console.log(
        `Sys: User disconnected with pending confirmation for ${socket.id}. Cancelling interaction.`,
      );
    ...
    if (questionResolverRef.value) {
      console.log(
        `Sys: User disconnected with pending question for ${socket.id}. Cancelling interaction.`,
      );
  ...
  socket.on("error", (err) => {
    ...
    emitLog(
      socket,
      `Critical socket error: ${err.message}. Please refresh.`,
      "error",
      true,
    );
    ...
    if (feedbackResolverRef.value) {
      console.warn(
        `Sys: Socket error with pending confirmation for ${socket.id}. Cancelling interaction.`,
      );
      if (typeof feedbackResolverRef.value === "function") {
    ...
    if (questionResolverRef.value) {
      console.warn(
        `Sys: Socket error with pending question for ${socket.id}. Cancelling interaction.`,
      );
      if (typeof questionResolverRef.value === "function") {
    ...
    confirmAllRef.value = false;
    console.log(
      `🧹 Cleared connection state refs for ${socket.id} due to socket error.`,
    );
  });
